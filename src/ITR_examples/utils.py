...
def collect_company_contributions(
    ...
    company_contributions = pd.DataFrame(
        data={
            ...
            "contribution": relative_contributions,
    ...
    company_contributions = company_contributions.merge(
        right=amended_portfolio.reset_index()[additional_columns],
        how="left",
        on="company_name",
    )
...
def anonymize(portfolio, provider):
    ...
    for index, company_name in enumerate(portfolio_companies):
        portfolio.loc[portfolio["company_name"] == company_name, "company_id"] = (
            "C" + str(index + 1)
        )
        portfolio.loc[portfolio["company_name"] == company_name, "company_isin"] = (
            "C" + str(index + 1)
        )
        ...
        portfolio.loc[portfolio["company_name"] == company_name, "company_name"] = (
            "Company" + str(index + 1)
        )
        provider.data["fundamental_data"].loc[
            provider.data["fundamental_data"]["company_name"] == company_name,
