...
class DataWarehouse(ABC):
            ...
            for scope_name in EScope.get_scopes():
                    ...
                    setattr(
                        ...
                        (
                            type(intensity_scope)(
                                ei_metric=intensity_scope.ei_metric, projections=intensity_scope.projections.copy()
                            )
                        ),
                    )
                ...
                if target_scope is not None:
                    setattr(
                        ...
                        (
                            type(target_scope)(
                                ei_metric=target_scope.ei_metric, projections=target_scope.projections.copy()
                            )
                        ),
                    )
                        ...
                        ei_list = list(
                            map(
                                lambda em_p: IEIRealization(
                                    year=em_p[0].year,
                                    value=Q_(np.nan, f"({em_p[0].value.u}) / ({em_p[1].value.u})")
                        ...
                        if data[primary_scope_attr]:
                            ...
                            if len(pre_s3_data) > 1:
                                pre_s3_data = list(
                                    map(
                                        lambda x: type(x)(
                                            year=x.year, value=data.S3[0].value * x.value / pre_s3_data[-1].value
                                        ),
                                        pre_s3_data[:-1],
                                    )
                                )
                    ...
                    def _align_and_sum_projected_targets(targets, primary_scope_attr):
                        ...
                        if isinstance(s3_projections, pd.Series):
                            ...
                            targets[primary_scope_attr].projections = list(
                                map(ICompanyEIProjection.add, primary_projections, s3_projections)
                            )
    ...
    def _process_company_data(
        ...
        df_trajectory = DataWarehouse._get_cumulative_emissions(
            projected_ei=projected_trajectories, projected_production=projected_production
        )
        ...
        df_company_data[ColumnsConfig.BENCHMARK_GLOBAL_BUDGET] = pd.Series(
            PA_([global_budget.m] * len(df_company_data), dtype=str(global_budget.u)), index=df_company_data.index
        )
    ...
    def _get_cumulative_emissions(cls, projected_ei: pd.DataFrame, projected_production: pd.DataFrame) -> pd.DataFrame:
        ...
        proj_CO2e_m_t = proj_prod_t.combine(
            proj_ei_t,
            lambda prod, ei: asPintSeries(ITR.data.osc_units.align_production_to_bm(prod, ei).mul(ei)).pint.m_as(
                units_CO2e
            ),
        )
        if ITR.HAS_UNCERTAINTIES:
    ...
    def _get_exceedance_year(
        ...
        if len(missing_subjects):
            df_aligned = pd.concat(
                [df_aligned, pd.Series(data=[pd.NA] * len(missing_subjects), index=missing_subjects)]
            )
