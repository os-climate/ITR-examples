...
def direct_conversions(ureg, unit) -> [str]:
    ...
    return [name for name in ureg if name != "%" and unit.dimensionality == unit_dimensionality(ureg, name)]
    ...
    try:
        ...
        if exp == -1:
            ...
            if errors == "ignore":
                if exp < 0:
                    x_implied_annual = Q_(
                        x * ureg("a").to(time_unit), unit_ct.remove([time_unit_str]).add(time_unit_str, exp + 1)
                    )
                else:
                    x_implied_annual = Q_(
                        x / ureg(time_unit_str).to("a"), unit_ct.remove([time_unit_str]).add(time_unit_str, exp - 1)
                    )
            raise DimensionalityError(
                x, "", extra_msg=f"; dimensionality must contain [time] or 1/[time], not [time]**{exp}"
            )
...
def dimension_as(x, dim_unit):
    ...
    try:
        unit, exp = next(
            (pint.Unit(unit), exp) for unit, exp in unit_ct.items() if ureg(unit).is_compatible_with(dim_unit)
        )
...
def align_production_to_bm(prod_series: pd.Series, ei_bm: pd.Series) -> pd.Series:
    ...
    if "[mass]" not in ureg.parse_units(ei_unit_top).dimensionality:
        try:
            mass_unit = [
                unit
                for unit, exp in pint.util.to_units_container(prod_series.iloc[0].to_base_units()).items()
...
def quantity(dimensionality: str) -> type:
    ...
    def validate(cls, value):
        if isinstance(value, str):
            ...
            raise TypeError(
                f"quantity takes either a Q_ value or a string fully expressing a quantified value; got {value}"
            )
        if quantity.is_compatible_with(dimensionality):
            return quantity
        assert quantity.check(
            cls.dimensionality
        ), f"Dimensionality of {quantity} incompatible with {cls.dimensionality}"
        return quantity

    @classmethod
    def __modify_schema__(cls, field_schema):
        field_schema.update({"$ref": "#/definitions/Quantity"})
...
class EmissionsQuantity(Quantity):
    ...
    def validate(cls, quantity):
        ...
        if quantity.is_compatible_with("t CO2"):
            return quantity
        raise DimensionalityError(
            quantity, "t CO2", dim1="", dim2="", extra_msg=f"Dimensionality must be compatible with 't CO2'"
        )
