...
def translate_currency_symbols_1(text):
    ...
    retval = "".join(
        map(lambda x: f"{x[0]}{'USD' if x[1]=='$' or x[1]=='US$' else currency_dict.get(x[1], '')}", pairs)
    )
...
unit_registry.preprocessors = [lambda s1: re.sub(r"passenger.km", "pkm", s1), lambda s2: translate_currency_symbols(s2)]
