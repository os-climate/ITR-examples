...
def convert_benchmarks_ei_excel_to_model(
    ...
    for index, row in df_ei_bms.iterrows():
        bm = IBenchmark(
            ...
            projections=[IProjection(year=int(k), value=Q_(float(v), index[2])) for k, v in row.items()],
        )
...
class ExcelProviderProductionBenchmark(BaseProviderProductionBenchmark):
    def __init__(self, excel_path: str, column_config: Type[ColumnsConfig] = ColumnsConfig):
        ...
        production_bms = self._convert_excel_to_model(
            self.benchmark_excel, TabsConfig.PROJECTED_PRODUCTION, column_config.SECTOR, column_config.REGION
        )
        super().__init__(IProductionBenchmarkScopes(AnyScope=production_bms), column_config)
...
class ExcelProviderCompany(BaseCompanyDataProvider):
    ...
    def _convert_from_excel_data(self, excel_path: str) -> List[ICompanyData]:
        ...
        df_fundamentals[ColumnsConfig.PRODUCTION_METRIC] = df_fundamentals[ColumnsConfig.SECTOR].map(
        ...
        df_targets = self._get_projection(
            company_ids, company_data[TabsConfig.PROJECTED_TARGET], df_fundamentals[ColumnsConfig.PRODUCTION_METRIC]
        )
        if TabsConfig.PROJECTED_EI in company_data:
            df_ei = self._get_projection(
                company_ids, company_data[TabsConfig.PROJECTED_EI], df_fundamentals[ColumnsConfig.PRODUCTION_METRIC]
            )
        else:
            df_ei = None
        if TabsConfig.HISTORIC_DATA in company_data:
            ...
            if False:
                ...
                df_historic = pd.concat([df_historic, df_s1s2, df_s1s2_ei, df_ei_from_em, df_em_from_ei])
            df_historic = df_historic.merge(
                df_fundamentals[ColumnsConfig.PRODUCTION_METRIC].rename("units"), left_index=True, right_index=True
            )
            ...
            df_historic.loc[df_historic.variable == "Emissions Intensities", "units"] = df_historic.loc[
    ...
    def _company_df_to_model(
        self, df_fundamentals: pd.DataFrame, df_targets: pd.DataFrame, df_ei: pd.DataFrame, df_historic: pd.DataFrame
    ) -> List[ICompanyData]:
        """
    ...
    def _get_projection(
        self, company_ids: List[str], projections: pd.DataFrame, production_metric: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        projections = projections.loc[
            company_ids, range(self.projection_controls.BASE_YEAR, self.projection_controls.TARGET_YEAR + 1)
        ]
    ...
    def _get_historic_data(self, company_ids: List[str], historic_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for year in self.historic_years:
            historic_data[year] = historic_data.apply(lambda x: f"{x[year]} {x.units}", axis=1)
        ...
        for scope_name in EScope.get_scopes():
            ...
            emissions_scopes[scope_name] = (
                ...
                if results.empty
                    ...
                    IEmissionRealization(year=year, value=EmissionsQuantity(results[year].squeeze()))
                    for year in self.historic_years
                ]
            )
        ...
        for scope_name in EScope.get_scopes():
            ...
            intensity_scopes[scope_name] = (
                ...
                if results.empty
                    ...
                    IEIRealization(year=year, value=EI_Quantity(results[year].squeeze()))
                    for year in self.historic_years
                ]
            )
