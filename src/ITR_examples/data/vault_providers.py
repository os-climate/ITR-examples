...
def create_table_from_df(
    df: pd.DataFrame, schemaname: str, tablename: str, engine: sqlalchemy.engine.base.Engine, verbose=False
):
...
def read_quantified_sql(
    sql: str, tablename, schemaname, engine: sqlalchemy.engine.base.Engine, index_col=None
) -> pd.DataFrame:
...
""",
    ...
    def compute_portfolio_weights(
        self, pa_temp_scores: pd.Series, year: int, factor: str, scope: EScope = EScope.S1S2
    ) -> pd.Series:
        """
        ...
        for scope in ["AnyScope"]:
            ...
            for benchmark in production_benchmarks.dict()[scope]["benchmarks"]:
                bdf = pd.DataFrame.from_dict(
                    {
                        r["year"]: [r["value"], benchmark["region"], benchmark["sector"], scope]
...
class VaultProviderIntensityBenchmark(IntensityBenchmarkDataProvider):
    ...
    def _get_intensity_benchmarks(
        ...
        benchmark_projection = benchmark_projection.loc[
            ...
            range(self.temp_config.CONTROLS_CONFIG.base_year, self.temp_config.CONTROLS_CONFIG.target_end_year + 1),
        ]
...
class DataVaultWarehouse(DataWarehouse):
    def __init__(
        ...
        qres = osc._do_sql(
            f"drop table if exists {self._schema}.{itr_prefix}cumulative_emissions", self._engine, verbose=False
        )
        qres = osc._do_sql(
            f"""
...
""",
        ...
        qres = osc._do_sql(
            f"drop table if exists {self._schema}.{itr_prefix}cumulative_budget_1", self._engine, verbose=False
        )
        qres = osc._do_sql(
            f"""
...
""",
    ...
    def quant_init(
        ...
        qres = osc._do_sql(
            f"drop table if exists {self._schema}.{itr_prefix}overshoot_ratios", self._engine, verbose=False
        )
        qres = osc._do_sql(
            f"""
