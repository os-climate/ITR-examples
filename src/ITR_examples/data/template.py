...
def prioritize_submetric(x: pd.DataFrame) -> pd.Series:
    ...
    while not x.empty:
        ...
        x = x[~submetric_idx]
        if len(z) > 1:
            z = z.groupby(by=[ColumnsConfig.SECTOR, ColumnsConfig.COMPANY_ID, "metric"], dropna=False).agg(
                _estimated_value
            )
        if y.empty:
            y = z
...
class TemplateProviderCompany(BaseCompanyDataProvider):
    ...
    def _solve_intensities(self, df_fundamentals: pd.DataFrame, df_esg: pd.DataFrame) -> pd.DataFrame:
        ...
        has_reported_production_mask = pd.Series(list(zip(*[df_esg[c] for c in compare_cols]))).isin(
            list(zip(*[df_esg[df_esg.metric.eq("production")][c] for c in compare_cols]))
        )
        ...
        if len(df_esg_missing_production):
            ...
            df_intensities = df_intensities.reset_index(drop=False).set_index(
                [ColumnsConfig.COMPANY_ID, "metric", ColumnsConfig.TEMPLATE_REPORT_DATE, "submetric"]
            )
            ...
            df_productions_t = df_productions_t.apply(
                lambda x: x.astype(f"pint[{ureg(str(x.dtype.units)).to_reduced_units().u}]")
            ...
            df_esg.loc[production_idx] = pd.concat(
                [df_esg.loc[production_idx].iloc[:, 0:start_year_loc], df_productions], axis=1
            )
        ...
        if len(df_esg_has_intensity):
            ...
            df1 = df_esg[df_esg.metric.eq("production")].set_index(
                [ColumnsConfig.COMPANY_ID, ColumnsConfig.TEMPLATE_REPORT_DATE, "submetric"]
            )
            if "boundary" in df1.columns:
                df1.drop(columns="boundary", inplace=True)
            df1 = df1.groupby(
                by=[ColumnsConfig.COMPANY_ID, ColumnsConfig.TEMPLATE_REPORT_DATE, "submetric"], dropna=False
            ...
            df1_case2 = (
                ...
                .drop(columns="sub_count")
                .merge(
                    df2.reset_index([ColumnsConfig.SCOPE, "index"])[[ColumnsConfig.SCOPE, "index"]],
                    left_index=True,
                    right_index=True,
                )
    ...
    def _init_from_template_company_data(self, excel_path: str):
        ...
        if self.template_version == 2:
            ...
            df_esg.rename(
                columns=lambda x: int(x) if isinstance(x, str) and x >= "1000" and x <= "2999" else x, inplace=True
            )
            ...
            with warnings.catch_warnings():
                ...
                df_esg.loc[
                    :, [ColumnsConfig.COMPANY_NAME, ColumnsConfig.COMPANY_ID, ColumnsConfig.TEMPLATE_REPORT_DATE]
                ] = df_esg[
                    [ColumnsConfig.COMPANY_NAME, ColumnsConfig.COMPANY_ID, ColumnsConfig.TEMPLATE_REPORT_DATE]
                ].ffill()
        ...
        if ColumnsConfig.TEMPLATE_FX_QUOTE in df_fundamentals.columns:
            ...
            if (
                len(df_fundamentals.loc[~fx_quote, ColumnsConfig.COMPANY_CURRENCY].unique()) > 1
                ...
                fx_df = (
                    df_fundamentals.loc[fx_quote, ["report_date", "currency", "fx_quote", "fx_rate"]]
                    .drop_duplicates()
                ...
                for col in fundamental_metrics:
                    ...
                    df_fundamentals[col] = (
                        df_fundamentals[col]
                        .astype("float64")
                        .astype(f"pint[{df_fundamentals[ColumnsConfig.COMPANY_CURRENCY].iloc[0]}]")
                    )
        else:
        ...
        df_fundamentals.drop(
            columns=["MCap_to_Reven", "MCap_to_Assets", "AVG_MCap_to_Reven", "AVG_MCap_to_Assets"], inplace=True
    ...
    def _convert_from_template_company_data(self) -> List[ICompanyData]:
        ...
        if self.template_version == 1:
            ...
            df = pd.wide_to_long(
                df_historic, historic_scopes, i="company_id", j="year", sep="-", suffix=r"\d+"
            ).reset_index()
            df2 = (
                    ...
                    df3.xs(VariablesConfig.PRODUCTIONS, level=1, drop_level=False).apply(
                        lambda x: x.map(
                            lambda y: Q_(
                                ...
                                df_fundamentals.loc[
                                    df_fundamentals.company_id == x.name[0], "production_metric"
                                ].squeeze(),
                            )
                        ),
                        axis=1,
                    ...
                    df3.xs(VariablesConfig.EMISSIONS, level=1, drop_level=False).apply(
                        lambda x: x.map(
                            lambda y: Q_(
                                float(y) if y is not pd.NA else np.nan,
                                df_fundamentals.loc[
                                    df_fundamentals.company_id == x.name[0], "emissions_metric"
                                ].squeeze(),
                            )
                        ),
                        axis=1,
            ...
            with warnings.catch_warnings():
                ...
                for col in esg_year_columns:
                    df_esg[col] = df_esg[col].combine(
                        u_col, lambda m, u: PintType(u).na_value if ITR.isna(m) else Q_(float(m), u)
                    )
            ...
            grouped_prod = (
                ...
                .groupby(by=[ColumnsConfig.SECTOR, ColumnsConfig.COMPANY_ID, "metric", "submetric"], dropna=False)[
                    esg_year_columns
                ]
            ...
            grouped_prod.submetric = pd.Categorical(
                ...
                + ["operated", "own", "revenue", "equity", "", "total", "gross", "net", "full", "*unrecognized*"],
            )
            ...
            if company_sector_idx.empty:
                ...
                new_fundamentals = (
                    ...
                    .assign(sector=new_company_ids.sector, company_id=new_company_ids.new_company_id)
                ...
                df_fundamentals = pd.concat(
                    [df_fundamentals[~df_fundamentals.company_id.isin(new_company_ids.company_id)], new_fundamentals]
                )
                ...
                case_4_df = case_1.reset_index("metric").merge(
                    case_2.reset_index("metric"), on=["sector", "company_id"], suffixes=[None, "_2"]
                )
            ...
            df3 = (
                pd.concat([best_prod, new_prod, best_esg_em, new_em_to_allocate, new_em_allocated])
    ...
    def _validate_target_data(self, target_data: pd.DataFrame) -> pd.DataFrame:
        ...
        with warnings.catch_warnings():
            ...
            target_data = target_data.assign(
                target_scope=target_data.target_scope.replace(r"[\n\r]+", "+", regex=True)
    ...
    def _company_df_to_model(
        self, df_fundamentals: pd.DataFrame, df_target_data: pd.DataFrame, df_historic_data: pd.DataFrame
    ) -> List[ICompanyData]:
        """
            ...
            try:
                ...
                if not df_historic_data.empty:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if False:
                                        ...
                                        df_historic_data.loc[(company_id, "Emissions", "S1S2"), :] = (
                                            df_historic_data.loc[company_id, "Emissions", "S1S2S3"] * 0
                                        )
                                        df_historic_data.loc[(company_id, "Emissions Intensities", "S1S2"), :] = (
                                            df_historic_data.loc[company_id, "Emissions Intensities", "S1S2S3"] * 0
                                        )
                                except KeyError:
                                    logger.error(
                    ...
                    company_data[ColumnsConfig.HISTORIC_DATA] = dict(
                        self._convert_historic_data(df_historic_data.loc[[company_id]].reset_index())
                    )
                else:
    ...
    def _convert_historic_data(self, historic: pd.DataFrame) -> IHistoricData:
        ...
        hd = IHistoricData(
            productions=self._convert_to_historic_productions(historic_t[VariablesConfig.PRODUCTIONS]),
        ...
        for scope_name, emissions in emissions_t.items():
            if not emissions.empty:
                emissions_scopes[scope_name] = [
                    IEmissionRealization(
                        year=year, value=TemplateProviderCompany._squeeze_NA_to_nan(emissions.loc[year])
                    )
                    for year in self.historic_years
                ]
    ...
    def _convert_to_historic_productions(self, productions_t: pd.DataFrame) -> Optional[List[IProductionRealization]]:
        ...
        if productions_t.empty:
            return None
        return [
            IProductionRealization(year=year, value=TemplateProviderCompany._squeeze_NA_to_nan(productions_t.loc[year]))
            for year in self.historic_years
        ]
        ...
        for scope_name, intensities in intensities_t.items():
            if not intensities.empty:
                intensity_scopes[scope_name] = [
                    IEIRealization(year=year, value=TemplateProviderCompany._squeeze_NA_to_nan(intensities.loc[year]))
                    for year in self.historic_years
                ]
